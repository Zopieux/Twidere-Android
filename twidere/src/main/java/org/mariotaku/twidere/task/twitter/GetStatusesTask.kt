...
abstract class GetStatusesTask(protected val context: Context) : AbstractTask<RefreshTaskParam, List<TwitterWrapper.StatusListResponse>, Any>(), Constants {
    ...
    @Inject
    lateinit protected var wrapper: AsyncTwitterWrapper
    ...
    private fun storeStatus(accountKey: UserKey, credentials: ParcelableCredentials,
        ...
        if (!statuses.isEmpty()) {
            val firstSortId = statuses[0].sortId
            val lastSortId = statuses[statuses.size - 1].sortId
        ...
        val deleteWhereArgs = arrayOfNulls<String>(statusIds.size + 1)
        System.arraycopy(statusIds, 0, deleteWhereArgs, 1, statusIds.size)
