 ...
 */
abstract class GetActivitiesTask(protected val context: Context) : AbstractTask<RefreshTaskParam, Any, Any>() {

    @Inject
    lateinit protected var preferences: KPreferences
    @Inject
    lateinit protected var bus: Bus
    @Inject
    lateinit protected var errorInfoStore: ErrorInfoStore
    @Inject
    lateinit protected var readStateManager: ReadStateManager
    @Inject
    lateinit protected var userColorNameManager: UserColorNameManager
    ...
    private fun storeActivities(cr: ContentResolver, loadItemLimit: Int, credentials: ParcelableCredentials,
        ...
        if (!activities.isEmpty()) {
            val firstSortId = activities[0].createdAt.time
            val lastSortId = activities[activities.size - 1].createdAt.time
        ...
        val writeUri = UriUtils.appendQueryParameters(contentUri, QUERY_PARAM_NOTIFY,
                notify)
    ...
    protected abstract fun saveReadPosition(accountKey: UserKey, credentials: ParcelableCredentials,
                                            twitter: MicroBlog)

    @Throws(MicroBlogException::class)
    protected abstract fun getActivities(twitter: MicroBlog, credentials: ParcelableCredentials,
