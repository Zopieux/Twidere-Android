...
class ExtensionsListLoader(context: Context) : AsyncTaskLoader<List<ExtensionsListLoader.ExtensionInfo>>(context), Constants {
    ...
    override fun loadInBackground(): List<ExtensionInfo> {
        ...
        return apps.filter {
            return@filter it.metaData?.getBoolean(METADATA_KEY_EXTENSION, false) ?: false
        }.map {
            return@map ExtensionInfo(it, packageManager)
        }
    }
    ...
    class ExtensionInfo internal constructor(info: ApplicationInfo, pm: PackageManager) : Comparable<ExtensionInfo> {
        ...
        init {
            ...
            permissions = if (permissionString != null)
                permissionString.split(SEPARATOR_PERMISSION_REGEX.toRegex()).dropLastWhile({ it.isEmpty() }).toTypedArray()
            else
                null
        ...
        override fun compareTo(other: ExtensionInfo): Int {
