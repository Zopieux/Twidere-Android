...
object EmojiSupportUtils {
    ...
    @JvmOverloads fun applyEmoji(manager: ExternalThemeManager, text: Spannable,
        ...
        while (arrayIdx >= 0) {
            val codePoint = array.get(arrayIdx)
            if (isEmoji(codePoint)) {
                ...
                if (arrayIdx > 0) {
                    val prevCodePoint = array.get(arrayIdx - 1)
                ...
                if (spans.size == 0) {
                    var drawable: Drawable? = emoji.getEmojiDrawableFor(*array.subarray(arrayIdx + arrayIdxOffset,
                            arrayEnd))
                    if (drawable == null) {
