...
class StreamingService : Service(), Constants {
    ...
    private fun setTwitterInstances(): Boolean {
        val accountsList = DataStoreUtils.getCredentialsList(this, true)
        val accountKeys = Array(accountsList.size) { accountsList[it].account_key }
        ...
        while (i < j) {
            ...
            val account = accountsList[i]
            val endpoint = MicroBlogAPIFactory.getEndpoint(account, TwitterUserStream::class.java)
            val authorization = MicroBlogAPIFactory.getAuthorization(account)
            val twitter = MicroBlogAPIFactory.getInstance(this, endpoint, authorization!!, TwitterUserStream::class.java)
            ...
            object : Thread() {
                override fun run() {
                    ...
                    Log.d(LOGTAG, String.format("Stream %s disconnected", account.account_key))
    ...
    internal class TwidereUserStreamCallback(private val context: Context, private val account: ParcelableAccount) : UserStreamCallback() {
        ...
        override fun onDirectMessage(directMessage: DirectMessage?) {
            ...
            val whereArgs = arrayOf(account.account_key.toString(), directMessage.id)
            for (uri in MESSAGES_URIS) {
                resolver.delete(uri, where, whereArgs)
            }
            val sender = directMessage.sender
            val recipient = directMessage.recipient
            if (TextUtils.equals(sender.id, account.account_key.id)) {
            ...
            if (TextUtils.equals(recipient.id, account.account_key.id)) {
                val values = ContentValuesCreator.createDirectMessage(directMessage,
        ...
        override fun onFollow(source: User, followedUser: User) {
            val message = String.format("%s followed %s", source.screenName, followedUser.screenName)
            Log.d(LOGTAG, message)
        }
        ...
        override fun onStatus(status: Status) {
            val values = ContentValuesCreator.createStatus(status, account.account_key)
            ...
            val whereArgs = arrayOf(account.account_key.toString(), status.id.toString())
            ...
            if (rt != null && rt.extendedText.contains("@" + account.screen_name) || rt == null && status.extendedText.contains("@" + account.screen_name)) {
